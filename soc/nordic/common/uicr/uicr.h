...
#define UICR_PERIPHCONF_ADD(_regptr, _value)                                                       \
	STRUCT_SECTION_ITERABLE(                                                                   \
		...
		_UICR_PERIPHCONF_ENTRY_NAME(UICR_PERIPHCONF_PREFIX, __COUNTER__)) = {              \
		.regptr = (_regptr),                                                               \
		.value = (_value),                                                                 \
	}
 ...
 * @param _index Register index into PERIPH[n].
...
/* TODO: needs to be feature specific? */
#define UICR_SPU_FEATURE_SET(_spu, _index, _secattr, _lock, _ownerid)                              \
	UICR_PERIPHCONF_ADD(                                                                       \
		((uint32_t *)&((NRF_SPU_Type *)(_spu))->FEATURE)[(_index)],                        \
 ...
 * @param _pin Pin number - register index into PIN_CNF[n].
 ...
 * @param _ppib_ch PPIB channel - register index into SUBSCRIBE_SEND[n].
 */
 ...
 *
 * @param _ppib Global domain PPIB instance.
 * @param _ppib_ch PPIB channel - register index into PUBLISH_RECEIVE[n].
 */
