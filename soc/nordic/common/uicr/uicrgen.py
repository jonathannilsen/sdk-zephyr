...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=None,
        type=argparse.FileType("w", encoding="utf-8"),
        help="Optional path to write a C source file representation of the generated blobs to",
    )
    ...
    try:
        ...
        try:
            ...
            raise ScriptError(
                "Failed to find PERIPHCONF partition in the devicetree. "
        ...
        periphconf_hex.frombytes(periphconf_combined, offset=periphconf_address)

        if kconfig["CONFIG_NRF_HALTIUM_UICR_PERIPHCONF"] == "y":
        ...
        if args.out_uicr_source is not None:
            source_content = generate_uicr_source_code(
                bytes(uicr_hex.tobinarray()), bytes(periphconf_hex.tobinarray())
            )
            print(source_content, file=args.out_uicr_source)
...
def parse_kconfig(content: str) -> dict[str, str | None]:
    ...
    match_iter = re.finditer(
        r"^(?P<config>CONFIG_[^=\s]+)=(?P<value>[^\s#])+", content, re.MULTILINE
    )
...
def extract_and_combine_periphconfs(elf_files: list[argparse.FileType]) -> bytes:
    ...
    for in_file in elf_files:
        ...
        if conf_section is None:
            raise ScriptError(f"Failed to find section '{PERIPHCONF_SECTION}' in {in_file.name}")
...
def generate_uicr_source_code(uicr_data: bytes, periphconf_data: bytes) -> str:
    uicr_hexdata_str = format_hexdata(uicr_data)
    periphconf_hexdata_str = format_hexdata(periphconf_data)
    ...
    return dedent(
        ...
        #define UICR_REGION LINKER_DT_NODE_REGION_NAME(DT_NODELABEL({UICR_NODELABEL}))
        #define PERIPHCONF_REGION LINKER_DT_NODE_REGION_NAME(DT_NODELABEL({PERIPHCONF_NODELABEL}))

        static const uint8_t uicr_data[] __used Z_GENERIC_SECTION(UICR_REGION) = {{
        %s
        }};

        static const uint8_t periphconf_data[] __used Z_GENERIC_SECTION(PERIPHCONF_REGION) = {{
        %s
        }};"""
    ) % (uicr_hexdata_str, periphconf_hexdata_str)


def format_hexdata(data, per_line: int = 16) -> str:
    lines = []
