...
/* XXX: could use the two functions below if the init API should not be
 * modified to automatically enable/load whenever a progress_key is specified.
 * I'm not sure if that behavior is desirable or not, or if changes to the init
...
/* XXX: could remove these functions as well if automatic progress
 * saving/loading is sufficient. The clear functionality could then maybe
 * be put into a stream_flash_finish() / stream_flash_reset() that can
 * is called when the user is done with their usage of the module.
 */
